...
while True:
    ...
    # Load and process the next frame on 'n' press
    if key == ord('n'):
        print("Hi")
        url = 'http://192.168.1.9:8080/photo.jpg'

        # Fetch image from ipcam
        try:
            req = urllib.request.urlopen(url)
            arr = np.asarray(bytearray(req.read()), dtype=np.uint8)
            photo = cv2.imdecode(arr, -1)  # Decode image

            if photo is None:
                print("Failed to decode image. Check the URL or camera.")
                continue

        except Exception as e:
            print(f"Error fetching image: {e}. Check network or URL.")
            continue

        # Detect license plates
        license_plates = license_plate_detector(photo, verbose=False)[0]
        ...
        cv2.rectangle(photo, (int(x1), int(y1)),
                      (int(x2), int(y2)), (0, 0, 255), 2)
        cv2.putText(photo, f'License Plate', (int(x1), int(
            y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

        # Crop license plate
        license_plate_crop = photo[int(y1):int(y2), int(x1):int(x2), :]
        ...
        cv2.imshow('Vehicle and License Plate Detection', photo)
        cv2.waitKey(1)
        ...
        cv2.putText(photo, license_plate_text, (text_x, text_y),
